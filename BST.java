...
public class BST<T> {
	...
	// Return a list containing all data (result is in increasing order
	// of the key)
	public LinkedList<T> getAllData() {
		LinkedList<T> l = new LinkedList<T>();
		recCollectInorderData(root, l);
		return l;
	}

	private void recCollectInorderData(BSTNode<T> t, LinkedList<T> l) {
		if (t == null) {
			return;
		}
		recCollectInorderData(t.left, l);
		l.insert(t.data);
		recCollectInorderData(t.right, l);
	}
